  ...
  return (
    <div className="screen">
      ...
      <div className="row">
        ...
        <button type="button" onClick={playlists.reload} disabled={playlists.state.status === "loading"}>
          Reload cached list
        </button>
      ...
      {playlists.state.status === "success" && rows.length === 0 ? (
        <p className="muted">
          Nothing cached yet. Sync from Spotify to fetch your playlists — the list is stored locally,
          so it loads instantly next time.
        </p>
      ) : null}
      ...
      {rows.length > 0 ? (
        <div className="table-scroll">
          <table className="data-table">
            <caption>{rows.length} playlists in the local cache</caption>
            <thead>
              <tr>
              ...
              {rows.map((playlist) => {
                ...
                return (
                  <tr key={playlist.spotifyId} className={isSelected ? "row-selected" : undefined}>
                    ...
                    <td>{dateTime(playlist.syncedAt)}</td>
                    <td>
                      <div className="row">
                        <button
                          type="button"
